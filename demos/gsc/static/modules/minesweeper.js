...
export function generateRandomPollution(n_pollution, map, wind_strength, wind_angle) {
...
export function checkClick(latlng, map) {
  ...
  grid_fm.forEach((fm) => {
    ...
    const double_threshold = 2 * distance_threshold
    if (dist < distance_threshold) {
      F.gameNotePopover('Got one!', 800)

      console.debug(`Found pollution! ${dist} degrees away`)
      fm.marker.addTo(map)
      fm.visible = true
      found += 1
    } else if (dist < double_threshold) {
      F.gameNotePopover('Very Close', 800)
    }
  })
  return found
}
