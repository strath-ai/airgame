  ...
  setWind(wind_strength, wind_angle, map) {
    ...
    if (wind_strength == 0) {
      ...
      this.zones = [
        createTriangle(
          this.latlng,
          dispersion_zone_scale,
          "green",
          wind_strength,
        ),
        createTriangle(
          this.latlng,
          dispersion_zone_scale / 2,
          "orange",
          wind_strength,
        ),
        createTriangle(
          this.latlng,
          dispersion_zone_scale / 4,
          "red",
          wind_strength,
        ),
      ];

      this.zones = this.zones.map((t) =>
        rotateTriangle(t, wind_angle, this.latlng, map),
      );
    }
  }
...
function rotateTriangle(triangle, angle, pivot = null, map) {
  return L.polygon(
    rotatePixelPolygon(
      triangle,
      angle,
      (pivot = pivot || pollution_source[0].getLatLng()),
      map,
    ),
    triangle.options,
  );
